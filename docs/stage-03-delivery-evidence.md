...
- Target: `staging`
- Candidate source hash: `3675d4acb9bcb78c5427741e3d833f28f27bf397` (exact pushed PR head; evidence captures were generated from the pre-packaging source tree and are separately identified below)
...
| Refreshed source integrity | `/opt/data/tmp/wdos-stage3/verified-ui/manifest.json`, `/opt/data/tmp/wdos-stage3/recovery-visual/machine-capture-manifest.json` | Both manifests identify the capture source tree as `e1be979c...ffb1`; this is pre-packaging evidence. The exact PR head is `3675d4a...f27bf397`; the post-capture delta is explicitly covered by focused tests and hosted CI, with no template/static/UI asset changes. |
...
revision binding, operator provisioning documentation, and regression coverage;
it does not alter the approved onboarding templates, static assets, locale
catalogs, or screenshot geometry. Hosted CI run `36034829729` passed both
Django and browser jobs on the preceding exact candidate; the current head
requires its own hosted run before readiness.
